...
async function getApikey(id) {
  ...
  let tiers = await dataweb.findOne();
  let users = await User.findOne({ _id: id })
  return { apikey: users.apikey, username: users.username, checklimit: users.limitApikey, checktier: tiers.tier, isVerified: users.isVerified, RequestToday: limit.RequestToday };
}
...
router.get('/docs', checkAuth, async (req, res) => {
  ...
  let { apikey, username, checklimit, isVerified, checktier, RequestToday } = getinfo
  res.render("docs", { username: username, verified: isVerified, tier: checktier, apikey: apikey, limit: checklimit, RequestToday: RequestToday });
